...
sending it:
  ...
  isms = ISMS.new("http://isms.example.com:81", "user", "password")
  response = isms.send_message("8002324000", "hi thar\nit's me")

  # response is now a hash containing the key "id" with the associated
  # SMS ID sent back from the iSMS device.

  puts isms.query_message_status(response['id'])
