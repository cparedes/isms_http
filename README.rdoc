...
sending it:
  ...
  # response is now an array with a bunch of hashes containing the key "id"
  # with the associated SMS ID sent back from the iSMS device.  there's
  # multiple hashes, because we sometimes have to split the message
  # into multiple parts, thanks to the 160 char limit.

  response.each do |item|
    puts isms.query_message_status(item['id']).inspect
  end
